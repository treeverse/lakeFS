...
func mockReadRows(_ context.Context, _ s3iface.S3API, inventoryBucketName string, file File) ([]FileRow, error) {
...
func TestFetch(t *testing.T) {
	...
	for _, test := range testdata {
		...
		if len(inv.rows) != len(test.ExpectedObjects) {
			t.Fatalf("unexpected number of objects in inventory. expected=%d, got=%d", len(test.ExpectedObjects), len(inv.rows))
		}
		if !reflect.DeepEqual(keys(inv.rows), test.ExpectedObjects) {
			t.Fatalf("objects in inventory differrent than expected. expected=%v, got=%v", test.ExpectedObjects, keys(inv.rows))
		}
	}
}
...
func TestLoadManifest(t *testing.T) {
	svc := mockS3Client{}
	inv := NewInventory(svc, "s3://example-bucket/example-path/manifest.json")
	err := inv.LoadManifest()
	if err != nil {
		t.Fatalf("failed to fetch manifest: %v", err)
	}
	if len(inv.Manifest().Files) == 0 {
...
func TestDiff(t *testing.T) {
	...
	for _, test := range data {
		diff := InventoryDiff(test.LeftInv, test.RightInv)
