...
CREATE VIEW public.lineage_v AS
...
CREATE VIEW public.entries_lineage_v AS
...
FROM (SELECT l.branch_id       AS displayed_branch,
      ...
      FROM (public.entries e
               JOIN public.lineage_v l ON ((l.ancestor_branch = e.branch_id)))
      WHERE ((l.main_branch OR (e.min_commit <= l.effective_commit)) AND (l.max_commit IS NULL))) t
WHERE (t.rank = 1);


CREATE VIEW public.entries_lineage_active_v AS
...
CREATE TABLE public.object_dedup
(
    repository_id    integer           NOT NULL,
...
ALTER TABLE ONLY public.object_dedup
...
CREATE UNIQUE INDEX branches_repository_name_uindex ON public.branches USING btree (name, repository_id);


CREATE INDEX entries_key_index ON public.entries USING btree (branch_id, key) INCLUDE (min_commit, max_commit);


CREATE INDEX entries_key_trgm ON public.entries USING gin (key public.gin_trgm_ops);


CREATE INDEX entries_size_index ON public.entries USING btree (size);


CREATE UNIQUE INDEX repositories_name_uindex ON public.repositories USING btree (name);


ALTER TABLE ONLY public.branches
    ADD CONSTRAINT branches_repository_id_fkey FOREIGN KEY (repository_id) REFERENCES public.repositories (id);


ALTER TABLE ONLY public.commits
    ADD CONSTRAINT commits_branches_repository_id_fk FOREIGN KEY (branch_id) REFERENCES public.branches (id) ON DELETE CASCADE;


ALTER TABLE ONLY public.lineage
    ADD CONSTRAINT lineage_branches_repository_id_fk FOREIGN KEY (branch_id) REFERENCES public.branches (id) ON DELETE CASCADE;


ALTER TABLE ONLY public.lineage
    ADD CONSTRAINT lineage_branches_repository_id_fk_2 FOREIGN KEY (ancestor_branch) REFERENCES public.branches (id) ON DELETE CASCADE;

ALTER TABLE ONLY public.multipart_uploads
    ADD CONSTRAINT multipart_uploads_repository_id_fkey FOREIGN KEY (repository_id) REFERENCES public.repositories (id) ON DELETE CASCADE;

ALTER TABLE ONLY public.object_dedup
    ADD CONSTRAINT object_dedup_repository_id_fkey FOREIGN KEY (repository_id) REFERENCES public.repositories (id) ON DELETE CASCADE;
