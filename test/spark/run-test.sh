...
if [ -v USE_DIRECT_ACCESS ]; then # Direct access using thick Spark client.
    docker-compose run -T --no-deps --rm spark-submit sh -c 'spark-submit --master spark://spark:7077 --jars ./target/client.jar -c "spark.hadoop.fs.lakefs.impl=io.lakefs.LakeFSFileSystem" --conf=spark.driver.extraJavaOptions=-Dcom.amazonaws.services.s3.enableV4 -c spark.hadoop.lakefs.api.url=s3.docker.lakefs.io:8000 -c "spark.hadoop.fs.lakefs.access.key=${TESTER_ACCESS_KEY_ID}" -c "spark.hadoop.fs.lakefs.secret.key=${TESTER_SECRET_ACCESS_KEY}" -c "spark.hadoop.fs.s3a.access.key=${NESSIE_AWS_ACCESS_KEY_ID}" -c "spark.hadoop.fs.s3a.secret.key=${NESSIE_AWS_SECRET_ACCESS_KEY}" --class Sonnets /local/app/target/scala-2.12/sonnets_2.12-0.1.jar ${input} ${output}'
