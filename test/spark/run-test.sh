...
#   STORAGE_NAMESPACE - storange namespace used for repository creation
# 
...
docker-compose exec -T lakefs lakectl repo create lakefs://example ${STORAGE_NAMESPACE} -d master

echo "==> upload dataset"
docker-compose exec -T lakefs lakectl fs upload -s /local/app/data-sets/sonnets.txt lakefs://example/master/sonnets.txt

echo "==> submit spark app"
docker-compose run -T --no-deps --rm spark-submit spark-submit --master spark://spark:7077 -c "spark.hadoop.fs.s3a.access.key=${TESTER_ACCESS_KEY_ID}" -c "spark.hadoop.fs.s3a.secret.key=${TESTER_SECRET_ACCESS_KEY}" --class Sonnets /local/app/target/scala-2.12/sonnets_2.12-0.1.jar
