...
if [ -v USE_DIRECT_ACCESS ]; then # Direct access using thick Spark client.
    ...
    -c spark.hadoop.fs.lakefs.impl=io.lakefs.LakeFSFileSystem\
     ...
     -c spark.hadoop.fs.lakefs.endpoint='${LAKEFS_URL:-http://lakefs:8000}'/api/v1\
    ...
    -c spark.hadoop.fs.s3a.endpoint='${LAKEFS_S3_ENDPOINT_URL:-s3.docker.lakefs.io:8000}'\
    -c spark.hadoop.fs.s3a.connection.ssl.enabled=false\
    --class Sonnets /local/app/target/'${SONNET_JAR}' '${s3input}' '${s3output}''
