...
trap 'clean_repo ${REPOSITORY}' INT TERM EXIT
...
jq -c '.[]' gc-tests/test_scenarios.json | while read test_case;
do
  echo "Test: $(echo "${test_case}" | jq -r '.description')"
