...
run_gc_according_to_storage_provider() {
  ...
  if [[ ${LAKEFS_BLOCKSTORE_TYPE} == "azure" ]]; then
    docker-compose run -v ${CLIENT_JAR}:/client/client.jar -T --no-deps --rm spark-submit "/spark/bin/spark-submit -v --packages org.apache.hadoop:hadoop-azure:3.2.1 --master spark://spark:7077 \
    ...
    docker-compose run -v ${CLIENT_JAR}:/client/client.jar -T --no-deps --rm spark-submit bash -c "/spark/bin/spark-submit -v --master spark://spark:7077 --class io.treeverse.clients.GarbageCollector \
