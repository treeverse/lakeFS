...
func TestLakectlFsStat(t *testing.T) {
	...
	t.Run("default", func(t *testing.T) {
		...
		if config.PreSignSupport {
			...
			if config.BlockstoreType == "s3" || config.BlockstoreType == "gs" || config.BlockstoreType == "azure" {
...
func TestLakectlUsage(t *testing.T) {
	...
	RunCmdAndVerifyContainsText(t, r.Replace("{lakectl} usage summary"), false, "Usage report for installation ID:", vars)
}
