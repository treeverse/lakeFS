...
func (s *mockSender) Send(ctx context.Context, installationId, processId string, m []stats.Metric) error {
	s.metrics <- m
	return nil
}
...
func TestCallHomeCollector_Collect(t *testing.T) {
	...
	collector := stats.NewBufferedCollector("installation_id", "process_id",
