...
func (p *Manager) SetEntry(ctx context.Context, st rocks.StagingToken, path rocks.Path, entry *rocks.Entry) error {
	...
	_, err := p.db.Transact(func(tx db.Tx) (interface{}, error) {
		return tx.Exec(`INSERT INTO staging_entries (staging_token, path, address, last_modified_date, size, checksum, metadata)
								VALUES ($1, $2, $3, $4, $5, $6, $7)
								ON CONFLICT (staging_token, path) DO UPDATE
									SET (staging_token, path, address, last_modified_date, size, checksum, metadata) =
											(excluded.staging_token, excluded.path, excluded.address, excluded.last_modified_date, excluded.size,
											 excluded.checksum, excluded.metadata)`,
...
func (p *Manager) ListEntries(ctx context.Context, st rocks.StagingToken) (rocks.EntryIterator, error) {
	return NewIterator(ctx, p.db, p.log, st), nil
}
