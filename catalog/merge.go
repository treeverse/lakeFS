...
func CompareEntries(a, b *Entry) (eqs int) {
	// names first
...
func prettyEntryNames(entries []*Entry) string {
...
func prettyWorkspaceNames(changes []*WorkspaceEntry) string {
...
func mergeChanges(current []*Entry, changes []*WorkspaceEntry, logger logging.Logger) ([]*Entry, error) {
	merged := make([]*Entry, 0)
	nextCurrent := 0
	nextChange := 0
	for {
