...
func makeHours(hours int) *catalog.TimePeriodHours {
	ret := catalog.TimePeriodHours(hours)
	return &ret
}

func readEntriesToExpire(t *testing.T, ctx context.Context, c catalog.Cataloger, repository string, policy *catalog.Policy) ([]*catalog.ExpireResult, error) {
	rows, err := c.QueryEntriesToExpire(ctx, repository, policy)
	if err != nil {
		t.Fatalf("scan for expired failed: %s", err)
	}
	defer rows.Close()
	ret := make([]*catalog.ExpireResult, 0, 10)
	for rows.Next() {
		e, err := rows.Read()
		if err != nil {
...
func less(a, b *catalog.ExpireResult) bool {
...
func sortExpireResults(results []*catalog.ExpireResult) {
...
type expiryTestCase struct {
	...
	want   []*catalog.ExpireResult
}

func verifyExpiry(t *testing.T, ctx context.Context, c catalog.Cataloger, repository string, tests []expiryTestCase) {
...
func TestCataloger_ScanExpired(t *testing.T) {
	...
	if _, err := c.Commit(ctx, repository, "master", "first commit", "tester", catalog.Metadata{}); err != nil {
	...
	if _, err := c.Commit(ctx, repository, "master", "second commit", "tester", catalog.Metadata{}); err != nil {
		t.Fatal("Failed to commit second commit to master", err)
	}

	if err := c.CreateEntry(ctx, repository, "slow", catalog.Entry{
		Path:            "0/committed",
	...
	if _, err := c.Commit(ctx, repository, "slow", "first slow commit", "tester", catalog.Metadata{}); err != nil {
		t.Fatal("Failed to commit to slow", err)
	}

	if err := c.CreateEntry(ctx, repository, "fast", catalog.Entry{
		Path:            "0/historical",
	...
	if _, err := c.Commit(ctx, repository, "fast", "first fast commit", "tester", catalog.Metadata{}); err != nil {
	...
	if _, err := c.Commit(ctx, repository, "fast", "second fast commit", "tester", catalog.Metadata{}); err != nil {
	...
	// minCommit.
	allResults, err := readEntriesToExpire(t, ctx, c, repository, &catalog.Policy{
		Rules: []catalog.Rule{
			{Enabled: true, FilterPrefix: "", Expiration: catalog.Expiration{All: makeHours(0)}},
		},
	})
	if err != nil {
		t.Fatalf("read all expiration records failed: %s", err)
	}
	resultByPhysicalAddress := make(map[string]*catalog.ExpireResult, len(allResults))
	for _, result := range allResults {
		t.Logf("Result: %+v", result)
		resultByPhysicalAddress[result.PhysicalAddress] = result
	}
	translate := func(physicalAddress string) *catalog.ExpireResult {
				...
				Rules: []catalog.Rule{
					{
						Enabled:      true,
						FilterPrefix: "",
						Expiration:   catalog.Expiration{All: makeHours(50)},
					},
				},
			},
			want: []*catalog.ExpireResult{},
				...
				Rules: []catalog.Rule{
					{
						Enabled:      true,
						FilterPrefix: "",
						Expiration:   catalog.Expiration{All: makeHours(0)},
					},
				},
			},
			want: []*catalog.ExpireResult{
				...
				Rules: []catalog.Rule{
					{
						Enabled:      true,
						FilterPrefix: "",
						Expiration:   catalog.Expiration{Uncommitted: makeHours(0)},
					},
				},
			},
			want: []*catalog.ExpireResult{masterUncommitted2Hours},
		}, {
				...
				Rules: []catalog.Rule{
					{
						Enabled:      true,
						FilterPrefix: "",
						Expiration:   catalog.Expiration{Noncurrent: makeHours(0)},
					},
				},
			},
			want: []*catalog.ExpireResult{
				...
				Rules: []catalog.Rule{
					{
						Enabled:      true,
						FilterPrefix: "",
						Expiration:   catalog.Expiration{Noncurrent: makeHours(18)},
					},
				},
			},
			want: []*catalog.ExpireResult{
...
func TestCataloger_ScanExpiredWithDupes(t *testing.T) {
	...
	resultByBranchAndPhysicalAddress := make(map[branchAndPhysicalAddress]*catalog.ExpireResult, len(allResults))
	for _, result := range allResults {
		t.Logf("Result: %+v", result)
		resultByBranchAndPhysicalAddress[branchAndPhysicalAddress{result.Branch, result.PhysicalAddress}] = result
	}
	translate := func(branch, physicalAddress string) *catalog.ExpireResult {
	...
	tests := []expiryTestCase{
		{
			...
			want: []*catalog.ExpireResult{masterDifferent},
		}, {
			name: "expire all",
			policy: &catalog.Policy{
				Rules: []catalog.Rule{
			...
			want: []*catalog.ExpireResult{masterHistorical, branchDifferent, masterDifferent},
		},
	}
	verifyExpiry(t, ctx, c, repository, tests)
}
...
func TestCataloger_MarkEntriesExpired(t *testing.T) {
	...
	if _, err := c.Commit(ctx, repository, "master", "commit ALL the files to expire", "tester", catalog.Metadata{}); err != nil {
		t.Fatalf("failed to commit: %s", err)
	}

	expireResults, err := readEntriesToExpire(t, ctx, c, repository, &catalog.Policy{
		Rules: []catalog.Rule{
			{Enabled: true, FilterPrefix: "", Expiration: catalog.Expiration{All: makeHours(0)}},
		},
	})
	...
	for _, e := range expireResults {
		...
		_, err = c.GetEntry(ctx, repository, "master", ref.Path, catalog.GetEntryParams{})
		if err == nil || !errors.Is(err, catalog.ErrExpired) {
			t.Errorf("didn't get expired entry %+v: %s", e, err)
		}

		entry, err := c.GetEntry(ctx, repository, "master", ref.Path, catalog.GetEntryParams{ReturnExpired: true})
...
func TestCataloger_MarkObjectsForDeletion(t *testing.T) {
	...
	type entryParams struct {
		entry  catalog.Entry
		params catalog.CreateEntryParams
	}

	makeDedup := func(id int) catalog.CreateEntryParams {
		return catalog.CreateEntryParams{
