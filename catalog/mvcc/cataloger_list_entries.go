...
func ListEntriesTx(tx db.Tx, branchID int64, commitID CommitID, prefix string, after string, limit int) ([]*catalog.Entry, error) {
	entriesSQL, args, err := ListEntriesQuery(tx, branchID, commitID, prefix, after, limit)
	if err != nil {
		return nil, fmt.Errorf("list entries query: %w", err)
	}
...
// ListEntriesQuery list entries query builder
func ListEntriesQuery(tx db.Tx, branchID int64, commitID CommitID, prefix string, after string, limit int) (string, []interface{}, error) {
