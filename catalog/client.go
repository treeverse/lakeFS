...
type ClientOperations interface {
	...
	ReadRepo(repoId string) (*Repo, error)
...
func (c *DBClientOperations) ReadRepo(repoId string) (*Repo, error) {
	repo := &Repo{}
	err := c.tx.Get(repo, `SELECT * FROM repositories WHERE id = $1`, repoId)
	return repo, err
}

func (c *DBClientOperations) ListRepos(amount int, after string) ([]*Repo, bool, error) {
	var repos []*Repo
	var hasMore bool
	var err error
	if amount < 0 {
...
func (c *DBClientOperations) WriteRepo(repo *Repo) error {
	_, err := c.tx.Exec(
