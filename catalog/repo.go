...
type RepoOperations interface {
	ReadRepo() (*Repo, error)
	LockWorkspace() error
	ListWorkspace(branch string) ([]*WorkspaceEntry, error)
	ListWorkspaceDirectory(branch, path, prefix, from string, amount int) ([]*WorkspaceEntry, bool, error)
	ListWorkspaceWithPrefix(branch, prefix, from string, amount int) ([]*WorkspaceEntry, bool, error)
	ReadFromWorkspace(branch, path string) (*WorkspaceEntry, error)
	ListBranches(prefix string, amount int, after string) ([]*Branch, bool, error)
	ReadBranch(branch string) (*Branch, error)
	ReadObject(addr string) (*Object, error)
	ReadCommit(addr string) (*Commit, error)
	ListTree(addr, after string, results int) ([]*Entry, bool, error)
	ListTreeWithPrefix(addr, prefix, after string, results int, afterInclusive bool) ([]*Entry, bool, error)
	ReadTreeEntry(treeAddress, name string) (*Entry, error)

	// Multipart uploads
	ReadMultipartUpload(uploadId string) (*MultipartUpload, error)
	ListMultipartUploads() ([]*MultipartUpload, error)
	GetObjectDedup(DedupId string) (*ObjectDedup, error)
	DeleteWorkspacePath(branch, path, typ string) error
	WriteToWorkspacePath(branch, parentPath, path string, entry *WorkspaceEntry) error
	ClearWorkspace(branch string) error
	WriteTree(address string, entries []*Entry) error
	WriteObject(addr string, object *Object) error
	WriteCommit(addr string, commit *Commit) error
	WriteBranch(name string, branch *Branch) error
	DeleteBranch(name string) error
	WriteRepo(repo *Repo) error
	WriteMultipartUpload(upload *MultipartUpload) error
	DeleteMultipartUpload(uploadId string) error
	WriteObjectDedup(dedup *ObjectDedup) error
}
...
func (o *DBRepoOperations) ReadRepo() (*Repo, error) {
	repo := &Repo{}
...
func (o *DBRepoOperations) ListWorkspace(branch string) ([]*WorkspaceEntry, error) {
...
func (o *DBRepoOperations) ListWorkspaceDirectory(branch, dir, prefix, from string, amount int) ([]*WorkspaceEntry, bool, error) {
	var entries []*WorkspaceEntry
	values := struct {
	...
	if len(from) > 0 {
		...
		values.AdditionalConditionDir = fmt.Sprintf("AND LEFT(path, LENGTH($3) + STRPOS(SUBSTRING(path FROM LENGTH($3) + 1), '%s')) > $5", Separator)
...
func (o *DBRepoOperations) ListWorkspaceWithPrefix(branch, prefix, from string, amount int) ([]*WorkspaceEntry, bool, error) {
	var entries []*WorkspaceEntry
	additionalCondition := ""
...
func (o *DBRepoOperations) ReadFromWorkspace(branch, path string) (*WorkspaceEntry, error) {
	ent := &WorkspaceEntry{}
...
func (o *DBRepoOperations) ListBranches(prefix string, amount int, after string) ([]*Branch, bool, error) {
	var err error
	var hasMore bool
	var branches []*Branch
...
func (o *DBRepoOperations) ReadBranch(branch string) (*Branch, error) {
	b := &Branch{}
...
func (o *DBRepoOperations) ReadObject(addr string) (*Object, error) {
	obj := &Object{}
	err := o.tx.Get(obj, `SELECT * FROM objects WHERE repository_id = $1 AND object_address = $2`, o.repoId, addr)
	return obj, err
}

func (o *DBRepoOperations) ReadCommit(addr string) (*Commit, error) {
	commit := &Commit{}
	var err error
...
func (o *DBRepoOperations) ListTreeWithPrefix(addr, prefix, after string, amount int, afterInclusive bool) ([]*Entry, bool, error) {
	var err error
	var hasMore bool
	var entries []*Entry
...
func (o *DBRepoOperations) ListTree(addr, after string, results int) ([]*Entry, bool, error) {
	return o.ListTreeWithPrefix(addr, "", after, results, false)
}

func (o *DBRepoOperations) ReadTreeEntry(treeAddress, name string) (*Entry, error) {
	entry := &Entry{}
...
func (o *DBRepoOperations) GetObjectDedup(dedupId string) (*ObjectDedup, error) {
	m := &ObjectDedup{}
...
func (o *DBRepoOperations) ReadMultipartUpload(uploadId string) (*MultipartUpload, error) {
	m := &MultipartUpload{}
...
func (o *DBRepoOperations) ListMultipartUploads() ([]*MultipartUpload, error) {
	var mpus []*MultipartUpload
...
func (o *DBRepoOperations) WriteToWorkspacePath(branch, parentPath, path string, entry *WorkspaceEntry) error {
	_, err := o.tx.Exec(`
...
func (o *DBRepoOperations) WriteObjectDedup(dedup *ObjectDedup) error {
...
func (o *DBRepoOperations) WriteTree(address string, entries []*Entry) error {
...
func (o *DBRepoOperations) WriteObject(addr string, object *Object) error {
...
func (o *DBRepoOperations) WriteCommit(addr string, commit *Commit) error {
	_, err := o.tx.Exec(`
...
func (o *DBRepoOperations) WriteBranch(name string, branch *Branch) error {
	_, err := o.tx.Exec(`
...
func (o *DBRepoOperations) WriteMultipartUpload(upload *MultipartUpload) error {
	_, err := o.tx.Exec(`
...
func (o *DBRepoOperations) WriteRepo(repo *Repo) error {
	_, err := o.tx.Exec(`
