...
func CommitScan(reader CommitReader, startAddr string, results int, after string) ([]*Commit, bool, error) {
...
func FindLowestCommonAncestor(reader CommitReader, addrA, addrB string) (*Commit, error) {
...
func findLowerCommonAncestorNextIter(discoveredSet map[string]struct{}, iter *CommitIterator) (*Commit, error) {
	if iter.Next() {
		commit := iter.Value()
