...
pub async fn diff_refs(configuration: &configuration::Configuration, repository: &str, left_ref: &str, right_ref: &str, after: Option<&str>, amount: Option<i32>, prefix: Option<&str>, delimiter: Option<&str>, r#type: Option<&str>, include_right_stats: Option<bool>) -> Result<models::DiffList, Error<DiffRefsError>> {
    ...
    if let Some(ref local_var_str) = include_right_stats {
        local_var_req_builder = local_var_req_builder.query(&[("include_right_stats", &local_var_str.to_string())]);
    }
...
pub async fn merge_into_branch_status(configuration: &configuration::Configuration, repository: &str, source_ref: &str, destination_branch: &str, id: &str) -> Result<models::MergeAsyncStatus, Error<MergeIntoBranchStatusError>> {
    let local_var_configuration = configuration;
