...
class S3Reader[Proto <: Message](val s3: AmazonS3, val bucket: String, val repositoryPrefix: String, messagePrototype: Proto) extends ProtoReader[Proto] {
  val reader = new SSTableReader

  override def get(rangeID: Array[Byte]): Seq[EntryRecord[Proto]] = {
    val localFile = File.createTempFile("lakefs", bucket)
