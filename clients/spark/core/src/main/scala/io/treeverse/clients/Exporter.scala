...
object Main {
  def main(args: Array[String]) {
    ...
    val connectionTimeout = sc.hadoopConfiguration.get(LAKEFS_CONF_API_CONNECTION_TIMEOUT)
    val readTimeout = sc.hadoopConfiguration.get(LAKEFS_CONF_API_READ_TIMEOUT)
    ...
    val apiClient = ApiClient.get(
      new APIConfigurations(endpoint, accessKey, secretKey, connectionTimeout, readTimeout)
    )
