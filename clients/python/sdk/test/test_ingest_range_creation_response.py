# coding: utf-8

"""
    lakeFS API

    lakeFS HTTP API

    The version of the OpenAPI document: 0.1.0
    Contact: services@treeverse.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

import lakefs_sdk
from lakefs_sdk.models.ingest_range_creation_response import IngestRangeCreationResponse  # noqa: E501
from lakefs_sdk.rest import ApiException

class TestIngestRangeCreationResponse(unittest.TestCase):
    """IngestRangeCreationResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test IngestRangeCreationResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `IngestRangeCreationResponse`
        """
        model = lakefs_sdk.models.ingest_range_creation_response.IngestRangeCreationResponse()  # noqa: E501
        if include_optional :
            return IngestRangeCreationResponse(
                range = lakefs_sdk.models.range_metadata.RangeMetadata(
                    id = '480e19972a6fbe98ab8e81ae5efdfd1a29037587e91244e87abd4adefffdb01c', 
                    min_key = 'production/collections/some/file_1.parquet', 
                    max_key = 'production/collections/some/file_8229.parquet', 
                    count = 56, 
                    estimated_size = 56, ), 
                pagination = lakefs_sdk.models.import_pagination.ImportPagination(
                    has_more = True, 
                    continuation_token = '', 
                    last_key = '', 
                    staging_token = '', )
            )
        else :
            return IngestRangeCreationResponse(
        )
        """

    def testIngestRangeCreationResponse(self):
        """Test IngestRangeCreationResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
