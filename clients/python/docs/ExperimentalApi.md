...
# **upload_part_from**
> UploadTo upload_part_from(repository, branch, upload_id, path, part_number, upload_part_from=upload_part_from)
...
with lakefs_sdk.ApiClient(configuration) as api_client:
    ...
    part_number = 56 # int | 
    upload_part_from = lakefs_sdk.UploadPartFrom() # UploadPartFrom |  (optional)

    try:
        api_response = api_instance.upload_part_from(repository, branch, upload_id, path, part_number, upload_part_from=upload_part_from)
        print("The response of ExperimentalApi->upload_part_from:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ExperimentalApi->upload_part_from: %s\n" % e)
```
 ...
 **path** | **str**|  | 
 **part_number** | **int**|  | 
 **upload_part_from** | [**UploadPartFrom**](UploadPartFrom.md)|  | [optional] 
