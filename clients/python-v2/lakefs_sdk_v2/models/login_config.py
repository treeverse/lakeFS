...
class LoginConfig(BaseModel):
    ...
    login_url_method: Optional[StrictStr] = Field(default=None, description="Defines login behavior when login_url is set. - none: For OSS users. - redirect: Auto-redirect to login_url. - select: Show a page to choose between logging in via login_url or with lakeFS credentials. Ignored if login_url is not configured. ")
    ...
    __properties: ClassVar[List[str]] = ["RBAC", "username_ui_placeholder", "password_ui_placeholder", "login_url", "login_url_method", "login_failed_message", "fallback_login_url", "fallback_login_label", "login_cookie_names", "logout_url"]
    ...
    def rbac_validate_enum(cls, value):
        ...
        if value not in set(['none', 'simplified', 'internal', 'external']):
            raise ValueError("must be one of enum values ('none', 'simplified', 'internal', 'external')")
        return value

    @field_validator('login_url_method')
    def login_url_method_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['none', 'redirect', 'select']):
