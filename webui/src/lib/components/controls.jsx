...
export const Loading = ({ message = 'Loading...' }) => {
    return (
        <div className="loading-container d-flex flex-column align-items-center justify-content-center py-5">
            <Spinner />
            <div className="loading-text text-center text-muted">{message}</div>
        </div>
    );
};
...
export const ActionsBar = ({ children }) => {
    return <div className="action-bar d-flex mb-3">{children}</div>;
};
...
export const LinkButton = ({ href, children, buttonVariant, tooltip = null }) => {
    if (tooltip === null) {
        return (
            <Link href={href} component={Button} variant={buttonVariant}>
                {children}
            </Link>
        );
    }
    return (
        <Link href={href} component={TooltipButton} tooltip={tooltip} variant={buttonVariant}>
            {children}
        </Link>
    );
};
...
export const RefreshButton = ({
    ...
    return (
        <TooltipButton tooltip={tooltip} variant={variant} onClick={onClick} size={size}>
            {icon}
        </TooltipButton>
    );
};
...
export const Warnings = ({ warnings = [] }) => {
    return (
        <ul className="pl-0 ms-0 warnings">
