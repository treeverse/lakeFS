...
const uploadFile = async (config, repo, reference, path, file, onProgress) => {
  ...
  if (config.pre_sign_support_ui) {
    ...
    const { status, etag } = await uploadWithProgress(
...
const UploadCandidate = ({
  ...
  return (
    <Container>
      <Row
        className={`upload-item upload-item-${state ? state.status : "none"}`}
      >
        <Col>
          <span className="path">
...
const UploadButton = ({
  ...
  const changeCurrentPath = useCallback(
    (e) => {
    ...
    <>
      <Modal size="xl" show={show} onHide={hide}>
          ...
          >
            ...
            <Form.Group controlId="content" className="mb-3">
              <div {...getRootProps({ className: "dropzone" })}>
                ...
                >
                  Drag &apos;n&apos; drop files or folders here (or click to
                  select)
                </div>
              </div>
                ...
                {files &&
                  files.map((file) => (
                    <UploadCandidate
                      ...
                      onRemove={
                        !uploadState.inProgress ? onRemoveCandidate(file) : null
                      }
                    />
                  ))}
              </aside>
        ...
        <Modal.Footer>
          ...
          <Button
            variant="success"
            disabled={uploadState.inProgress || files.length < 1}
            onClick={() => {
              if (uploadState.inProgress) return;
              upload();
            }}
