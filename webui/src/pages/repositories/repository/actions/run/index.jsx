...
const HookLog = ({ repo, run, execution }) => {
    ...
    return (
        <div className="hook-log">
            ...
            <div className="hook-log-content">{content}</div>
        </div>
    );
};
...
const ActionBrowser = ({ repo, run, hooks, onSelectAction, selectedAction = null }) => {
    ...
    // group by action
    const actionNames = {};
    hookRuns.forEach((hookRun) => {
        actionNames[hookRun.action] = true;
    });
        ...
        <Row className="mt-3">
            <Col md={{ span: 3 }}>
                <ListGroup variant="flush">
                    <ListGroup.Item action onClick={() => onSelectAction(null)}>
                        <HomeIcon /> Summary
                    </ListGroup.Item>
                </ListGroup>
