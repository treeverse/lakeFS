...
const LoginForm = ({loginConfig}: { loginConfig: LoginConfig }) => {
    ...
    const [loginError, setLoginError] = useState<string | JSX.Element | null>(
        null,
    );
    const {next} = router.query;
    const usernamePlaceholder =
        loginConfig.username_ui_placeholder || "Access Key ID";
    const passwordPlaceholder =
        loginConfig.password_ui_placeholder || "Secret Access Key";
    return (
        <Row>
                    ...
                    <Card.Body>
                        ...
                        <div className={"mt-2 mb-1"}>
                            {loginConfig.fallback_login_url ? (
                                ...
                                >
                                    {loginConfig.fallback_login_label ||
...
const LoginPage = () => {
    ...
    if (
        ...
        (setupResponse.state !== SETUP_STATE_INITIALIZED ||
            setupResponse.comm_prefs_missing === true)
    ) {
        router.push({
            pathname: "/setup",
            params: {},
