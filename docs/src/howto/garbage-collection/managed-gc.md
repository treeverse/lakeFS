...
{: .note}
> Managed GC is only available for [lakeFS Cloud](/cloud/index/). If you are using the self-managed lakeFS, garbage collection is [available to run manually](/howto/garbage-collection/index/).
...
## How it works
Similarly to the self-managed lakeFS, managed GC uses [garbage collection rules](/howto/garbage-collection/index/) to determine which objects to delete.
