...
## Prepare your GCS bucket with minimal permissions (Advanced)

If required, lakeFS can operate without accessing the data itself. This permission model is useful if you are using [presigned URLs mode][presigned-url] or the [lakeFS Hadoop FileSystem Spark integration][integration-hadoopfs].
...
3. **Configure Bucket IAM Policies**
   ...
   gcloud storage buckets add-iam-policy-binding "gs://$BUCKET" \
     ...
     --condition="title=lakefs-metadata-only,description=Access_only_to_lakefs_prefix,expression=resource.name.startsWith('projects/_/buckets/$BUCKET/objects/_lakefs/')"
   ```
   ...
   gcloud storage buckets add-iam-policy-binding "gs://$BUCKET" \
     ...
     --role="roles/storage.objectAdmin" \
     --condition="title=lakefs-data-only,description=Access_except_lakefs_prefix,expression=!resource.name.startsWith('projects/_/buckets/$BUCKET/objects/_lakefs/')"
   ```
...
4. **Set Up VPC Service Controls**
   ...
   basic:
     conditions:
     - ipSubnetworks:
       - "[YOUR_ALLOWED_IP_CIDR]"  # e.g., "10.0.0.0/8"
       vpcNetworkSources:
       - vpcSubnetwork:
...
6. **Create VPC Service Controls Perimeter**
...
7. **Configure lakeFS**
   ...
   In your lakeFS configuration, use the metadata service account (SA_OPEN) credentials:
...
[understand-commits]: ../../understand/how/versioning-internals.md#commits
