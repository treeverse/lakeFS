...
* `auth` - Configuration section for the Fluffy authentication services, like SAML or OIDC.
  ...
  + `auth.external` - Configuration section for the external authentication methods
    + `auth.external.aws_auth` - Configuration section for authenticating to lakeFS using AWS presign get-caller-identity request: [External Principals AWS Auth](/security/external-principals-aws/)
