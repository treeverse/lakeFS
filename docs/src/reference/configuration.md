...
=== "`database.dynamodb`"
    ...
    * `database.dynamodb.credentials_cache_expiry_window` `(duration : 60s)` - The expiry window for cached AWS credentials. This controls how long before credentials expire that the SDK will attempt to refresh them.
    * `database.dynamodb.credentials_cache_expiry_window_jitter` `(float : 0.5)` - The jitter fraction (0.0-1.0) for credentials cache expiry. This adds randomness to prevent thundering herd effects when refreshing credentials. A value of 0.5 means up to 50% jitter.
