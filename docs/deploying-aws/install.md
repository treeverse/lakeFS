...
1. Copy the Helm values file relevant to your cloud provider:
   ...
   Save the file as `conf-values.yaml`.
...
In order to perform actions on lakeFS, you can use the following APIs (these are also covered in the [Architecture](../architecture/overview.md) section):
...
The third domain (`*.s3.lakefs.example.com`) allows for [virtual-host style access](https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html). This is a way for AWS client to specify the bucket name in the Host subdomain.
