...
+ `email.limit_every` `(duration : 1m)` - Average time in seconds to wait between sending emails.
+ `email.burst` `(int: 1)` - Maximal burst of emails before applying `limit_every`.
