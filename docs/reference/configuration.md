...
+ `email.limit_every_duration` `(duration : 1m)` - Average time to wait between sending emails. The zero value means *infinite* duration and therefore *unlimited* emails can be sent.
+ `email.burst` `(int: 10)` - Maximal burst of emails before applying `limit_every`. The zero value means no burst and therefore no emails can be sent.
