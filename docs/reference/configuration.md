...
* `blockstore.gc.disable_pre_signed_ui` `(bool: false)` - When enabled the UI will use a pre-signed URL to get and upload objects from the UI. For it to work, need to add [cors permissions](https://cloud.google.com/storage/docs/using-cors) to the bucket:
   ```json
  [
    {
...
* `blockstore.s3.disable_pre_signed_ui` `(bool: false)` - When enabled the UI will use a pre-signed URL to get and upload objects from the UI. For it to work, need to add [cors permissions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/enabling-cors-examples.html) to the bucket:
   ```json
  [
    {
        "AllowedHeaders": [
