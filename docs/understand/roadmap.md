...
Committing (along with attaching useful information on the commit) makes a lot of sense for batch workloads: run a job or a pipeline on a separate branch, and then commit once the pipeline has completed successfully, recording metadata such as the git hash of the code executed, the versions of frameworks used, and information about the data artifacts generated by the run.
