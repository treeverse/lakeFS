...
var ingestCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		intgSrc := MustString(cmd.Flags().GetString("integration-source"))
		...
		go func() {
			err := store.Walk(ctx, intgSrc, from, func(e store.ObjectStoreEntry) error {
				if dryRun {
					Fmt("%s\n", e)
					return nil
				}
...
func init() {
	...
	ingestCmd.Flags().String("integration-source", "", "integration source to use for ingestion")
