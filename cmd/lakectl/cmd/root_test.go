	...
	for _, tt := range testCases {
		t.Run(tt.Name, func(t *testing.T) {
			...
			if len(expectedVal) > 0 {
				t.Setenv("LAKECTL_EXPERIMENTAL_LOCAL_UNIX_PERMISSIONS", expectedVal)
			}
			...
			if expectedVal == "true" {
				require.True(t, cfg.Experimental.Local.UnixPerm)
			} else {
				require.False(t, cfg.Experimental.Local.UnixPerm)
			}
		})
	}
}
