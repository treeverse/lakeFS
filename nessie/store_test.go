...
const IngestTestBacketPath = "s3://nessie-system-testing-data/ingest-test-data/"

func TestS3Walk(t *testing.T) {
	s3Client, err := store.GetS3Client(*aws.NewConfig().WithRegion("us-east-1"))
	require.NoError(t, err, "Failed to create S3 client")
	walker := &store.S3Walker{}
	walker.SetS3(s3Client)
	...
	uri, err := url.Parse(IngestTestBacketPath)
	require.NoError(t, err, "url parsing failed", IngestTestBacketPath)
