...
func TestCommit(t *testing.T) {
	...
	RunCmdAndVerifySuccessWithFile(t, Lakectl()+" fs upload -s files/ro_1k.txt lakefs://"+repoName+"/"+mainBranch+"/ro_1k.txt.1", false, "lakectl_fs_upload", vars)
	RunCmdAndVerifySuccessWithFile(t, Lakectl()+" log lakefs://"+repoName+"/"+mainBranch, false, "lakectl_log_initial", vars)
	RunCmdAndVerifyFailureWithFile(t, Lakectl()+" commit lakefs://"+repoName+"/"+mainBranch, false, "lakectl_commit_no_msg", vars)
	RunCmdAndVerifyFailureWithFile(t, Lakectl()+" commit lakefs://"+repoName+"/"+mainBranch+" -m \" \"", false, "lakectl_commit_empty_msg_err", vars)
	...
	RunCmdAndVerifySuccessWithFile(t, Lakectl()+" fs upload -s files/upload_file.txt lakefs://"+repoName+"/"+mainBranch+"/upload_file.txt.1", false, "lakectl_fs_second_upload", vars)
