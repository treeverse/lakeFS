...
func TestFindMergeBase(t *testing.T) {
	...
	for _, cas := range cases {
		t.Run(cas.Name, func(t *testing.T) {
			getter := cas.Getter()
			base, _, err := ref.FindMergeBase(context.Background(), getter, repository, cas.Left, cas.Right)
			if err != nil {
...
func TestGrid(t *testing.T) {
	...
	repository := &graveler.RepositoryRecord{RepositoryID: "ref-test-repo"}
	getter := newReader(kv)
	c, _, err := ref.FindMergeBase(context.Background(), getter, repository, "7-4", "5-6")
	testutil.Must(t, err)
	verifyResult(t, c, []string{"5-4"}, getter.visited)

	getter.visited = map[graveler.CommitID]int{}
	c, _, err = ref.FindMergeBase(context.Background(), getter, repository, "1-2", "2-1")
	testutil.Must(t, err)
	verifyResult(t, c, []string{"1-1"}, getter.visited)

	getter.visited = map[graveler.CommitID]int{}
	c, _, err = ref.FindMergeBase(context.Background(), getter, repository, "0-9", "9-0")
	testutil.Must(t, err)
	verifyResult(t, c, []string{"0-0"}, getter.visited)

	getter.visited = map[graveler.CommitID]int{}
	c, _, err = ref.FindMergeBase(context.Background(), getter, repository, "6-9", "9-6")
	testutil.Must(t, err)
