...
var (
	...
	ErrLinkAddressAlreadyExists     = fmt.Errorf("link address token already exists: %w", ErrNotUnique)
	ErrLinkAddressNotFound          = fmt.Errorf("link address token %w", ErrNotFound)
	ErralinkAddressExpired          = errors.New("link address token has expired")
