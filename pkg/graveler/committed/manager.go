...
func NewCommittedManager(m map[graveler.StorageID]MetaRangeManager, r map[graveler.StorageID]RangeManager, cr graveler.ConflictsResolver, p Params) graveler.CommittedManager {
	return &committedManager{
		metaRangeManagers: m,
		RangeManagers:     r,
		conflictsResolver: cr,
		params:            &p,
	}
}
...
func (c *committedManager) merge(ctx context.Context, mctx mergeContext) (graveler.MetaRangeID, error) {
	...
	err = Merge(mwWriter, c.conflictsResolver, ctx, oCtx, baseIt, srcIt, destIt, mctx.strategy)
	if err != nil {
