...
func TestUpdate(t *testing.T) {
	...
	update = func(settingsToEdit proto.Message) error {
		...
		require.NoError(t, m.Save(ctx, "example-repo", "settingKey", initialData))
		return nil
	}
	require.ErrorIs(t, m.Update(ctx, "example-repo", "settingKey", emptySettings, update), graveler.ErrPreconditionFailed)
...
func prepareTest(t *testing.T, ctx context.Context, kvEnabled bool, cache cache.Cache, branchLockCallback func(context.Context, graveler.RepositoryID, graveler.BranchID, func() (interface{}, error)) (interface{}, error)) (settings.Manager, block.Adapter) {
	...
	if kvEnabled {
		...
		m = settings.NewManager(refManager, kv.StoreMessage{Store: kvStore}, blockAdapter, "_lakefs", opts...)
	} else {
