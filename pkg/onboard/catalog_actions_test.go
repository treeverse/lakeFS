...
func TestFullCyclePorcelainImportBranchExists(t *testing.T) {
	...
	rangeManager.EXPECT().FlushToMetaRange(gomock.Any(), gomock.Eq(repoID), gomock.Any()).Times(1).Return(&mri, nil)
...
func TestApplyImportWriteFailure(t *testing.T) {
	...
	rangeManager.EXPECT().FlushToMetaRange(gomock.Any(), gomock.Eq(repoID), gomock.Any()).Times(1).Return(nil, errors.New("some failure"))
