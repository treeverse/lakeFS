...
func handleListMultipartUploads(w http.ResponseWriter, req *http.Request, o *RepoOperation) {
	...
	iter, err := o.MultipartTracker.List(req.Context(), partition)
	if err != nil {
	...
	for len(uploads) < int(maxUploads) && iter.Next() {
		upload := iter.Value()
		// Per AWS S3 spec: markers are excluded from results
		// Skip the exact marker if it's the first item after seeking
		if len(uploads) == 0 && keyMarker != "" && upload.Path == keyMarker && upload.UploadID == uploadIDMarker {
			continue
		}
