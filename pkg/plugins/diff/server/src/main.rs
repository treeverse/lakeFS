...
use differ::{DiffProps, table_differ_server::{TableDiffer, TableDifferServer}, DiffRequest, DiffResponse, GatewayConfig};
...
impl Differ for DifferService {
    async fn diff(&self, request: Request<DiffRequest>) -> Result<Response<DiffResponse>, Status> {
        ...
        let ps: DiffPaths = match r.props {
            Some(ps) => ps,
            None => return Ok(Response::new(DiffResponse { records: vec![], change_type: 0 }))
        };
