...
use differ::{
    table_differ_server::{TableDiffer, TableDifferServer},
    GatewayConfig, TableDiffRequest, TableDiffResponse,
};

use crate::differ::{
    DiffProps, DiffType, ShowHistoryRequest, ShowHistoryResponse, TableOperation, TablePath,
};
...
impl TableDiffer for DifferService {
    ...
    async fn table_diff(
        &self,
        request: Request<TableDiffRequest>,
    ) -> Result<Response<TableDiffResponse>, Status> {
        let r = request.into_inner();
        let s3_gateway_config_req: GatewayConfig = r
            .gateway_config
            .expect("Missing S3 compatible gateway configurations");
        let diff_props: DiffProps = r.props.expect("Missing diff properties");
        let left_table_path: TablePath = diff_props
            .left_table_path
            .expect("Missing left table's path");
        let right_table_path: TablePath = diff_props
            .right_table_path
            .expect("Missing right table's path");
        let s3_config_map: HashMap<String, String> =
            utils::construct_storage_config(s3_gateway_config_req);
        let left_table_res =
        ...
        let base_path = diff_props.base_table_path.unwrap_or(TablePath {
            r#ref: "".to_string(),
            path: "".to_string(),
        });
        let base_table_res =
        ...
        if right_table_res.is_err() || left_table_res.is_err() {
            match get_diff_type(&left_table_res, &right_table_res, &base_table_res).await {
                Ok(diff_type) => match diff_type {
                    DiffType::Dropped => {
                        return Ok(Response::new(TableDiffResponse {
                            entries: vec![],
                            diff_type: i32::from(DiffType::Dropped),
                        }))
                    }
                    DiffType::Created => {
                        ...
                        return Ok(Response::new(TableDiffResponse {
                            entries: ans,
                            diff_type: i32::from(DiffType::Created),
                        }));
                    }
                    DiffType::Changed => {
    ...
    async fn show_history(
        &self,
        _request: Request<ShowHistoryRequest>,
    ) -> Result<Response<ShowHistoryResponse>, Status> {
        todo!()
    }
}

async fn get_diff_type(
    left_table_res: &Result<DeltaTable, Status>,
    right_table_res: &Result<DeltaTable, Status>,
    base_table_res: &Result<DeltaTable, Status>,
) -> Result<DiffType, Status> {
        ...
        (Err(status), Ok(_)) => {
            ...
            if matches!(status.code(), Code::NotFound) {
                if base_table_res.is_err() {
                    if matches!(
                        base_table_res.as_ref().unwrap_err().code(),
                        Code::NotFound | Code::InvalidArgument
                    ) {
                        // didn't exist in base ref or no base ref provided
                        Ok(DiffType::Created)
                    } else {
                        // There was other kind of error with the base branch
                        Err(base_table_res.as_ref().unwrap_err().clone())
                    }
                } else {
                    // The table existed on the base branch
                    Ok(DiffType::Changed)
                }
            } else {
...
async fn run_diff(
    left_table: &mut DeltaTable,
    right_table: &mut DeltaTable,
) -> Result<Response<TableDiffResponse>, Status> {
    ...
    let mut left_history_version = HistoryAndVersion {
        history: left_table_history?,
        version: left_table.version(),
    };
    let mut right_history_version = HistoryAndVersion {
        history: right_table_history?,
        version: right_table.version(),
    };
    let diff = compare(&mut left_history_version, &mut right_history_version).unwrap();
    Ok(Response::new(TableDiffResponse {
        entries: diff,
        diff_type: i32::from(DiffType::Changed),
    }))
}
...
fn show_history(
    mut hist: Vec<CommitInfo>,
    mut table_version: DeltaDataTypeVersion,
) -> Result<Vec<TableOperation>, Status> {
    hist.reverse();
...
fn compare(
    left_history_version: &mut HistoryAndVersion,
    ...
    if right_table_version > left_table_version {
        ...
        for commit_info in right_iter {
            if curr_version == left_table_version {
                right_commit_slice = &right_history_version.history
                    [(right_table_version - left_table_version) as usize..];
                break;
            }
            match commit_info {
                _operation_content => {
        ...
        // If left table's version is bigger, start running over the left log from the right table's version.
        left_commit_slice =
            &left_history_version.history[(left_table_version - right_table_version) as usize..];
    }
    compare_table_slices(
        left_commit_slice,
        right_commit_slice,
        curr_version,
        common_ancestor,
...
fn construct_table_op(
    commit_info: &CommitInfo,
    version: DeltaDataTypeVersion,
) -> Result<TableOperation, Status> {
    match utils::construct_table_op(commit_info, version) {
        Ok(table_ops) => Ok(table_ops),
        Err(e) => Err(Status::new(
            Code::Aborted,
            format!("Creating operation history aborted due to:\n{:?}", e),
        )),
    }
}
