...
func RegisterServices(ctx context.Context, sd ServiceDependencies, router *chi.Mux) error {
	if hook := entensionhooks.Get().API; hook != nil {
		return hook.RegisterServices(ctx, entensionhooks.ServiceDependencies{
			Config:                sd.Config,
			AuthService:           sd.AuthService,
			Authenticator:         sd.Authenticator,
			AuthenticationService: sd.AuthenticationService,
			BlockAdapter:          sd.BlockAdapter,
			Collector:             sd.Collector,
			Logger:                sd.Logger,
			LicenseManager:        sd.LicenseManager,
		}, router)
	}
...
func BuildConditionFromParams(ifMatch, ifNoneMatch *string) (*graveler.ConditionFunc, error) {
	if hook := entensionhooks.Get().API; hook != nil {
		return hook.BuildConditionFromParams(ifMatch, ifNoneMatch)
	}
...
func NewIcebergSyncController(cfg config.Config) icebergsync.Controller {
	if hook := entensionhooks.Get().API; hook != nil {
		return hook.NewIcebergSyncController(cfg)
	}
