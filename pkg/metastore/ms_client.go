...
func copyOrMergeWithTransformLocation(ctx context.Context, fromClient, toClient Client, fromDB, fromTable, toDB, toTable, toInputFormat, serde string, partition []string, transformLocation func(location string) (string, error), fixSparkPlaceHolder bool) error {
	log := logging.Default().WithFields(logging.Fields{
		"from_db":         fromDB,
		"from_table":      fromTable,
		"to_db":           toDB,
		"to_table":        toTable,
		"to_input_format": toInputFormat,
		"serde":           serde,
		"partition_len":   len(partition),
	})
	if len(partition) > 0 {
		log.Debug("CopyPartition")
		return CopyPartition(ctx, fromClient, toClient, fromDB, fromTable, toDB, toTable, toInputFormat, serde, partition, transformLocation, fixSparkPlaceHolder)
	}
	...
	if !hasTable {
		...
		return Copy(ctx, table, partitions, toDB, toTable, toInputFormat, serde, toClient, transformLocation, fixSparkPlaceHolder)
	}
	log.Debug("Merge")
	table, err := fromClient.GetTable(ctx, fromDB, fromTable)
	if err != nil {
	...
	return Merge(ctx, table, partitionCollection, toDB, toTable, toInputFormat, serde, toClient, transformLocation, fixSparkPlaceHolder)
}
...
func Copy(ctx context.Context, fromTable *Table, partitions []*Partition, toDB, toTable, toInputFormat, serde string, toClient WriteClient, transformLocation func(location string) (string, error), fixSparkPlaceHolder bool) error {
	isSparkSQLTable := fromTable.isSparkSQLTable()
	err := fromTable.Update(toDB, toTable, toInputFormat, serde, transformLocation, isSparkSQLTable, fixSparkPlaceHolder)
	if err != nil {
		return err
	}
	for _, partition := range partitions {
		err := partition.Update(toDB, toTable, toInputFormat, serde, transformLocation, isSparkSQLTable, fixSparkPlaceHolder)
		if err != nil {
			return err
		}
	}
...
func Merge(ctx context.Context, table *Table, partitionIter Collection, toDB, toTable, toInputFormat, serde string, toClient Client, transformLocation func(location string) (string, error), fixSparkPlaceHolder bool) error {
	isSparkSQLTable := table.isSparkSQLTable()
	err := table.Update(toDB, toTable, toInputFormat, serde, transformLocation, isSparkSQLTable, fixSparkPlaceHolder)
	if err != nil {
		return err
	}
...
func CopyPartition(ctx context.Context, fromClient ReadClient, toClient Client, fromDB, fromTable, toDB, toTable, toInputFormat, serde string, partition []string, transformLocation func(location string) (string, error), fixSparkPlaceHolder bool) error {
	...
	err = p1.Update(toDB, toTable, toInputFormat, serde, transformLocation, t1.isSparkSQLTable(), fixSparkPlaceHolder)
	if err != nil {
		return err
	}
	if p2 == nil {
...
func CopyOrMergeToSymlink(ctx context.Context, fromClient, toClient Client, fromDB, fromTable, toDB, toTable, locationPrefix, toInputFormat string, fixSparkPlaceHolder bool) error {
	transformLocation := func(location string) (string, error) {
		return GetSymlinkLocation(location, locationPrefix)
	}
	return copyOrMergeWithTransformLocation(ctx, fromClient, toClient, fromDB, fromTable, toDB, toTable, toInputFormat, "", nil, transformLocation, fixSparkPlaceHolder)
}
