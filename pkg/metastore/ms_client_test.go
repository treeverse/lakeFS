...
func TestMSClient_CopyAndMergeBack(t *testing.T) {
	...
	err := metastore.CopyOrMerge(ctx, clientFrom, clientTo, dbName, tableName, toDBName, toTableName, toBranch, toTableName, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// now merge back
	err = metastore.CopyOrMerge(ctx, clientTo, clientFrom, toDBName, toTableName, dbName, tableName, branch, toTableName, nil)
	if err != nil {
		t.Fatal(err)
	}
