...
func BuildBlockAdapter(ctx context.Context, statsCollector stats.Collector, blockstoreTag *string, c config.AdapterConfig) (block.Adapter, error) {
	...
	case block.BlockstoreTypeS3:
		...
		return buildS3Adapter(ctx, statsCollector, p, blockstoreTag)
	...
	case block.BlockstoreTypeGS:
		...
		return buildGSAdapter(ctx, p, blockstoreTag)
	case block.BlockstoreTypeAzure:
		p, err := c.BlockstoreAzureParams()
		if err != nil {
			return nil, err
		}
		return azure.NewAdapter(ctx, p, blockstoreTag)
	default:
...
func buildS3Adapter(ctx context.Context, statsCollector stats.Collector, params params.S3, blockstoreTag *string) (*s3a.Adapter, error) {
	...
	adapter, err := s3a.NewAdapter(ctx, params, blockstoreTag, opts...)
	if err != nil {
		return nil, err
	}
...
func buildGSAdapter(ctx context.Context, params params.GS, blockstoreTag *string) (*gs.Adapter, error) {
	...
	adapter := gs.NewAdapter(client, blockstoreTag, opts...)
