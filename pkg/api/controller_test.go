...
func testCommitEntries(t *testing.T, ctx context.Context, cat *catalog.Catalog, deps *dependencies, params commitEntriesParams) string {
	...
	commit, err := cat.Commit(ctx, params.repo, params.branch, "commit"+params.commitName, params.user, nil, nil, nil, nil)
...
func TestController_LogCommitsParallelHandler(t *testing.T) {
	...
	for i := 0; i < commits; i++ {
		...
		err := deps.catalog.CreateEntry(ctx, repo, "main", catalog.DBEntry{Path: p, PhysicalAddress: onBlock(deps, "bar"+n+"addr"), CreationDate: time.Now(), Size: int64(i) + 1, Checksum: "cksum" + n})
		testutil.MustDo(t, "create entry "+p, err)
		log, err := deps.catalog.Commit(ctx, repo, "main", t.Name()+" commit"+n, "some_user", nil, nil, nil, nil)
		testutil.MustDo(t, "commit "+p, err)
		if i%4 == 0 {
			commitsToLook[p] = log
		}
...
func TestController_GetCommitHandler(t *testing.T) {
	...
	t.Run("get existing commit", func(t *testing.T) {
		...
		commit1, err := deps.catalog.Commit(ctx, "foo1", "main", "some message", DefaultUserID, nil, nil, nil, nil)
		testutil.Must(t, err)
	...
	t.Run("branch commit", func(t *testing.T) {
		...
		commit1, err := deps.catalog.Commit(ctx, repo, "main", "some message", DefaultUserID, nil, nil, nil, nil)
		testutil.Must(t, err)
...
func TestController_ListBranchesHandler(t *testing.T) {
	...
	t.Run("list branches pagination", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, repo, "main", "first commit", "test", nil, nil, nil, nil)
		testutil.Must(t, err)
...
func TestController_ListTagsHandler(t *testing.T) {
	...
	commitLog, err := deps.catalog.Commit(ctx, repo, "main", "first commit", "test", nil, nil, nil, nil)
	testutil.Must(t, err)
...
func TestController_GetBranchHandler(t *testing.T) {
	...
	t.Run("get default branch", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, repo, testBranch, "first commit", "test", nil, nil, nil, nil)
		testutil.Must(t, err)
...
func TestController_CreateBranchHandler(t *testing.T) {
	...
	t.Run("create branch and diff refs success", func(t *testing.T) {
		...
		if _, err := deps.catalog.Commit(ctx, repo, "main2", "commit 1", "some_user", nil, nil, nil, nil); err != nil {
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, repo, "main", "first commit", "test", nil, nil, nil, nil, graveler.WithForce(true))
		testutil.Must(t, err)
...
func TestController_DiffRefsHandler(t *testing.T) {
	...
	t.Run("diff prefix with and without delimiter", func(t *testing.T) {
		...
		if _, err := deps.catalog.Commit(ctx, repoName, newBranchName, "commit 1", "some_user", nil, nil, nil, nil); err != nil {
			t.Fatalf("failed to commit 'repo1': %s", err)
		}
...
func TestController_UploadObjectHandler(t *testing.T) {
	...
	t.Run("disable overwrite with if-none-match (committed entry)", func(t *testing.T) {
		...
		// commit
		_, err = deps.catalog.Commit(ctx, "my-new-repo", "another-branch", "a commit!", "user1", nil, nil, nil, nil)
		testutil.Must(t, err)
...
func TestController_DeleteBranchHandler(t *testing.T) {
	...
	t.Run("delete branch success", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, "my-new-repo", "main", "first commit", "test", nil, nil, nil, nil)
		testutil.Must(t, err)
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, repoName, "main", "first commit", "test", nil, nil, nil, nil, graveler.WithForce(true))
		testutil.Must(t, err)
...
func TestController_MergeIntoExplicitBranch(t *testing.T) {
	...
	testutil.Must(t, err)
	_, err = deps.catalog.Commit(ctx, repo, "branch1", "some message", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
...
func TestController_CreateTag(t *testing.T) {
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		commit1, err := deps.catalog.Commit(ctx, readOnlyRepo, "main", "some message", DefaultUserID, nil, nil, nil, nil, graveler.WithForce(true))
		testutil.Must(t, err)
...
func TestController_Revert(t *testing.T) {
	...
	testutil.Must(t, err)
	testutil.MustDo(t, "create entry bar1", deps.catalog.CreateEntry(ctx, repo, "main", catalog.DBEntry{Path: "foo/bar1", PhysicalAddress: "bar1addr", CreationDate: time.Now(), Size: 1, Checksum: "cksum1"}))
	_, err = deps.catalog.Commit(ctx, repo, "main", "some message", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
	...
	t.Run("revert_no_parent", func(t *testing.T) {
		...
		testutil.Must(t, err)
		_, err = deps.catalog.Commit(ctx, repo, "main", "first", DefaultUserID, nil, nil, nil, nil)
		testutil.Must(t, err)
		...
		testutil.Must(t, err)
		_, err = deps.catalog.Commit(ctx, repo, "branch1", "second", DefaultUserID, nil, nil, nil, nil)
		testutil.Must(t, err)
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, readOnlyRepository, "main", "some message", DefaultUserID, nil, nil, nil, nil, graveler.WithForce(true))
		testutil.Must(t, err)
...
func TestController_RevertConflict(t *testing.T) {
	...
	testutil.Must(t, err)
	testutil.MustDo(t, "create entry bar1", deps.catalog.CreateEntry(ctx, repo, "main", catalog.DBEntry{Path: "foo/bar1", PhysicalAddress: "bar1addr", CreationDate: time.Now(), Size: 1, Checksum: "cksum1"}))
	firstCommit, err := deps.catalog.Commit(ctx, repo, "main", "some message", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
	...
	_, err = deps.catalog.Commit(ctx, repo, "main", "some other message", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
...
func TestController_CherryPick(t *testing.T) {
	...
	testutil.Must(t, err)
	testutil.MustDo(t, "create entry bar1", deps.catalog.CreateEntry(ctx, repo, "main", catalog.DBEntry{Path: "foo/bar1", PhysicalAddress: "bar1addr", CreationDate: time.Now(), Size: 1, Checksum: "cksum1"}))
	_, err = deps.catalog.Commit(ctx, repo, "main", "message1", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
	...
	commit2, err := deps.catalog.Commit(ctx, repo, "branch1", "message2", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
	...
	_, err = deps.catalog.Commit(ctx, repo, "branch1", "message34", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
	...
	_, err = deps.catalog.Commit(ctx, repo, "branch2", "message34", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
	...
	_, err = deps.catalog.Commit(ctx, repo, "branch3", "message8", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
	...
	_, err = deps.catalog.Commit(ctx, repo, "branch4", "message4", DefaultUserID, nil, nil, nil, nil)
	testutil.Must(t, err)
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, readOnlyRepository, "branch1", "message2", DefaultUserID, nil, nil, nil, nil, graveler.WithForce(true))
		testutil.Must(t, err)
...
func TestController_PrepareGarbageCollectionUncommitted(t *testing.T) {
	...
	t.Run("committed_data", func(t *testing.T) {
		...
		if _, err := deps.catalog.Commit(ctx, repo, "main", "committed objects", "some_user", nil, nil, nil, nil); err != nil {
...
func TestController_DumpRestoreRepository(t *testing.T) {
	...
	for i := 0; i < commits; i++ {
		...
		err := deps.catalog.CreateEntry(ctx, repo, "main", catalog.DBEntry{Path: p, PhysicalAddress: onBlock(deps, "bar"+n+"addr"), CreationDate: time.Now(), Size: int64(i) + 1, Checksum: "cksum" + n})
		testutil.MustDo(t, "create entry "+p, err)
		_, err = deps.catalog.Commit(ctx, repo, "main", "commit"+n, "tester", nil, nil, nil, nil)
		testutil.MustDo(t, "commit "+p, err)
	}
