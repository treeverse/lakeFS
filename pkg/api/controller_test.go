...
func testCommitEntries(t *testing.T, ctx context.Context, cat *catalog.Catalog, deps *dependencies, params commitEntriesParams) string {
	...
	commit, err := cat.Commit(ctx, params.repo, params.branch, catalog.CommitParams{
		Message:         "commit" + params.commitName,
	...
	for _, ttt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < tt.commits; i++ {
				...
				testutil.MustDo(t, "create entry "+p, err)
				_, err = deps.catalog.Commit(ctx, repo, "main", catalog.CommitParams{
					Message:         "commit" + n,
					Committer:       "some_user",
					Metadata:        nil,
					SourceMetaRange: nil,
...
func TestController_LogCommitsParallelHandler(t *testing.T) {
	...
	for i := 0; i < commits; i++ {
		...
		err := deps.catalog.CreateEntry(ctx, repo, "main", catalog.DBEntry{Path: p, PhysicalAddress: onBlock(deps, "bar"+n+"addr"), CreationDate: time.Now(), Size: int64(i) + 1, Checksum: "cksum" + n})
		testutil.MustDo(t, "create entry "+p, err)
		log, err := deps.catalog.Commit(ctx, repo, "main", catalog.CommitParams{
			Message:         t.Name() + " commit" + n,
			Committer:       "some_user",
			Metadata:        nil,
			SourceMetaRange: nil,
			Date:            nil,
...
func TestController_LogCommitsPredefinedData(t *testing.T) {
	...
	for i := 0; i < totalCommits; i++ {
		...
		err := deps.catalog.CreateEntry(ctx, repo, "main", catalog.DBEntry{Path: p, PhysicalAddress: onBlock(deps, "bar"+n+"addr"), CreationDate: time.Now(), Size: int64(i) + 1, Checksum: "cksum" + n})
		testutil.MustDo(t, "create entry "+p, err)
		commit, err := deps.catalog.Commit(ctx, repo, "main", catalog.CommitParams{
			Message:         "commit" + n,
			Committer:       "some_user",
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_GetCommitHandler(t *testing.T) {
	...
	t.Run("get existing commit", func(t *testing.T) {
		...
		commit1, err := deps.catalog.Commit(ctx, "foo1", "main", catalog.CommitParams{
	...
	t.Run("branch commit", func(t *testing.T) {
		...
		commit1, err := deps.catalog.Commit(ctx, repo, "main", catalog.CommitParams{
			Message:         "some message",
			Committer:       DefaultUserID,
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_ListTagsHandler(t *testing.T) {
	...
	commitLog, err := deps.catalog.Commit(ctx, repo, "main", catalog.CommitParams{
		Message:         "first commit",
		Committer:       "test",
		Metadata:        nil,
		SourceMetaRange: nil,
...
func TestController_GetBranchHandler(t *testing.T) {
	...
	t.Run("get default branch", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, repo, testBranch, catalog.CommitParams{
			Message:         "first commit",
			Committer:       "test",
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_CreateBranchHandler(t *testing.T) {
	...
	t.Run("create branch and diff refs success", func(t *testing.T) {
		...
		if _, err := deps.catalog.Commit(ctx, repo, "main2", catalog.CommitParams{
			Message:         "commit 1",
			Committer:       "some_user",
			Metadata:        nil,
			SourceMetaRange: nil,
			Date:            nil,
			AllowEmpty:      false}); err != nil {
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, repo, "main", catalog.CommitParams{
			...
			SourceMetaRange: nil,
			Date:            nil,
			AllowEmpty:      false}, graveler.WithForce(true))
		testutil.Must(t, err)
...
func TestController_DiffRefsHandler(t *testing.T) {
	...
	t.Run("diff prefix with and without delimiter", func(t *testing.T) {
		...
		if _, err := deps.catalog.Commit(ctx, repoName, newBranchName, catalog.CommitParams{
			Message:         "commit 1",
			Committer:       "some_user",
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_UploadObjectHandler(t *testing.T) {
	...
	t.Run("disable overwrite with if-none-match (committed entry)", func(t *testing.T) {
		...
		// commit
		_, err = deps.catalog.Commit(ctx, "my-new-repo", "another-branch", catalog.CommitParams{
			Message:         "a commit!",
			Committer:       "user1",
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_DeleteBranchHandler(t *testing.T) {
	...
	t.Run("delete branch success", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, "my-new-repo", "main", catalog.CommitParams{
			Message:         "first commit",
			Committer:       "test",
			Metadata:        nil,
			SourceMetaRange: nil,
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, repoName, "main", catalog.CommitParams{
			Message:         "first commit",
			Committer:       "test",
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_MergeIntoExplicitBranch(t *testing.T) {
	...
	testutil.Must(t, err)
	_, err = deps.catalog.Commit(ctx, repo, "branch1", catalog.CommitParams{
		Message:         "some message",
		Committer:       DefaultUserID,
		Metadata:        nil,
		SourceMetaRange: nil,
...
func TestController_CreateTag(t *testing.T) {
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		commit1, err := deps.catalog.Commit(ctx, readOnlyRepo, "main", catalog.CommitParams{
			Message:         "some message",
			Committer:       DefaultUserID,
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_Revert(t *testing.T) {
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, readOnlyRepository, "main", catalog.CommitParams{
			Message:         "some message",
			Committer:       DefaultUserID,
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_RevertConflict(t *testing.T) {
	...
	testutil.Must(t, err)
	testutil.MustDo(t, "create entry bar1", deps.catalog.CreateEntry(ctx, repo, "main", catalog.DBEntry{Path: "foo/bar1", PhysicalAddress: "bar1addr", CreationDate: time.Now(), Size: 1, Checksum: "cksum1"}))
	firstCommit, err := deps.catalog.Commit(ctx, repo, "main", catalog.CommitParams{
		Message:         "some message",
		Committer:       DefaultUserID,
		Metadata:        nil,
		SourceMetaRange: nil,
...
func TestController_CherryPick(t *testing.T) {
	...
	commit2, err := deps.catalog.Commit(ctx, repo, "branch1", catalog.CommitParams{
		Message:         "message2",
		Committer:       DefaultUserID,
		Metadata:        nil,
		SourceMetaRange: nil,
	...
	_, err = deps.catalog.Commit(ctx, repo, "branch2", catalog.CommitParams{
		Message:         "message34",
		Committer:       DefaultUserID,
		Metadata:        nil,
		SourceMetaRange: nil,
	...
	_, err = deps.catalog.Commit(ctx, repo, "branch3", catalog.CommitParams{
		Message:         "message8",
		Committer:       DefaultUserID,
		Metadata:        nil,
		SourceMetaRange: nil,
	...
	_, err = deps.catalog.Commit(ctx, repo, "branch4", catalog.CommitParams{
		Message:         "message4",
		Committer:       DefaultUserID,
		Metadata:        nil,
		SourceMetaRange: nil,
	...
	t.Run("read-only repository", func(t *testing.T) {
		...
		_, err = deps.catalog.Commit(ctx, readOnlyRepository, "branch1", catalog.CommitParams{
			Message:         "message2",
			Committer:       DefaultUserID,
			Metadata:        nil,
			SourceMetaRange: nil,
...
func TestController_PrepareGarbageCollectionUncommitted(t *testing.T) {
	...
	t.Run("committed_data", func(t *testing.T) {
		...
		if _, err := deps.catalog.Commit(ctx, repo, "main", catalog.CommitParams{
			Message:         "committed objects",
			Committer:       "some_user",
			Metadata:        nil,
			SourceMetaRange: nil,
			Date:            nil,
