...
func TestController_GetRepoHandler(t *testing.T) {
	...
	t.Run("use same storage namespace", func(t *testing.T) {
...
func TestController_DeleteRepositoryHandler(t *testing.T) {
	...
	t.Run("delete read-only repository", func(t *testing.T) {
		repo := testUniqueRepoName()
		_, err := deps.catalog.CreateRepository(ctx, repo, config.SingleBlockstoreID, onBlock(deps, "foo1"), "main", true)
		testutil.Must(t, err)
...
func TestController_ObjectsDeleteObjectHandler(t *testing.T) {
	...
	repo := testUniqueRepoName()
	const branch = "main"
	_, err := deps.catalog.CreateRepository(ctx, repo, config.SingleBlockstoreID, onBlock(deps, "some-bucket/prefix"), branch, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("delete objects protected", func(t *testing.T) {
		...
		err = deps.catalog.SetBranchProtectionRules(ctx, repo, &graveler.BranchProtectionRules{
			BranchPatternToBlockedActions: rules,
		}, swag.String(config.SingleBlockstoreID))
		testutil.Must(t, err)
...
func TestController_ObjectsDeleteObjectHandler_Perf(t *testing.T) {
	clt, deps := setupClientWithAdmin(t)
	ctx := context.Background()
	...
	//for i := range 10000 {
	//	err = deps.catalog.CreateEntry(ctx, repo, branch,
	//		catalog.DBEntry{
	//			Path:            fmt.Sprintf("f_%d", i),
	//			PhysicalAddress: onBlock(deps, fmt.Sprintf("f_%d", i)),
	//			CreationDate:    time.Now(),
	//			Size:            0,
	//			Checksum:        "",
	//		})
	...
	for _, withNoTombstone := range []bool{false, true} {
		t.Run("delete object in committed stress", func(t *testing.T) {
			b := fmt.Sprintf("test_tombstone_%v", withNoTombstone)
			_, err = deps.catalog.CreateBranch(ctx, repo, b, branch)
			require.NoError(t, err)
			// delete
			for i := range 10000 {
				delResp, err := clt.DeleteObjectWithResponse(ctx, repo, b, &apigen.DeleteObjectParams{Path: fmt.Sprintf("f_%d", i), NoTombstone: (*apigen.NoTombstone)(swag.Bool(withNoTombstone))})
				verifyResponseOK(t, delResp, err)
			}
			_, err = deps.catalog.Commit(ctx, repo, branch, "commit", "test", nil, nil, nil, false)
			testutil.MustDo(t, "commit", err)
		})
	}
	...
	//	// delete
	//	for i := range 10000 {
	//		delResp, err := clt.DeleteObjectWithResponse(ctx, repo, branch, &apigen.DeleteObjectParams{Path: fmt.Sprintf("f_%d", i), NoTombstone: (*apigen.NoTombstone)(swag.Bool(true))})
	//		verifyResponseOK(t, delResp, err)
	//	}
	//})
}
