...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    status_code TASK_STATUS_CODE_VALUE NOT NULL, -- internal status code, used by parade to issue tasks
    num_tries INTEGER NOT NULL DEFAULT 0,
    max_tries INTEGER,
    actor_id VARCHAR(32),    -- ID of performing actor if in-progress
...
CREATE TABLE IF NOT EXISTS task_dependencies (
    after VARCHAR(32) REFERENCES tasks(id) -- after this task ID is done
        ON DELETE CASCADE ON UPDATE CASCADE,
    run VARCHAR(32) REFERENCES tasks(id)	-- run this task ID
        ON DELETE CASCADE ON UPDATE CASCADE
);
...
-- be allocated.
CREATE OR REPLACE FUNCTION can_allocate_task(id VARCHAR(32), code TASK_STATUS_CODE_VALUE, deadline TIMESTAMPTZ)
RETURNS BOOLEAN
...
CREATE OR REPLACE FUNCTION own_tasks(
    max_tasks INTEGER, actions VARCHAR(128) ARRAY, owner_id VARCHAR(32)
)
RETURNS TABLE(task_id VARCHAR(32), token UUID)
LANGUAGE sql VOLATILE AS $$
    UPDATE tasks
    SET actor_id = owner_id, status_code = 'in-progress', num_tries = num_tries + 1, performance_token = gen_random_uuid()
    WHERE id = (
        SELECT id
	FROM tasks
...
CREATE OR REPLACE PROCEDURE return_task(
    task_id VARCHAR(32), token UUID, result_status TEXT, result_status_code TASK_STATUS_CODE_VALUE
)
LANGUAGE plpgsql AS $$
DECLARE
    num_updated INTEGER;
BEGIN
    UPDATE tasks
    SET status = result_status, status_code = result_status_code, actor_id = NULL, performance_token = NULL
    WHERE id = task_id AND performance_token = token;

    GET DIAGNOSTICS num_updated := ROW_COUNT;
    IF num_updated != 1 THEN
        RAISE EXCEPTION 'found % tasks for task % with performance token % when setting status % code %; perhaps another task took it?', num_updated, task_id, token, result_status, result_status_code;
    END IF;
