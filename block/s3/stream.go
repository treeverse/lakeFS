...
func CalculateStreamSizeForPayload(sizeBytes int, chunkSize int) int {
	...
	fullChunks := sizeBytes / chunkSize
	partialChunkSize := sizeBytes % chunkSize

	overhead := fullChunks * (chunkSigSize + len(fmt.Sprintf("%x", chunkSize))) // one sig for every chunk
	if partialChunkSize > 0 {
		overhead += chunkSigSize + len(fmt.Sprintf("%x", partialChunkSize)) // last partial chunk if there is one
	}
	overhead += chunkSigSize + len(fmt.Sprintf("%x", 0)) // trailing sig
